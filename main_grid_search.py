...
if __name__ == '__main__':   
    ...
    lg = [2, 3, 4]
    lr = np.logspace(-5, -2, num=5)
    aggr = ['flatten', 'mean', 'sum', 'max']
    dropout = np.random.uniform(0.05, 0.5, 5)
    wd = np.logspace(-6, -3, num=5)
    hgc = [8, 16, 24]  # New hyperparameter

    excel_writer = pd.ExcelWriter(args.RES_DIR + 'results.xlsx', engine='xlsxwriter')

    best_accuracy = 0
    best_params = None

    # Initialize row index for writing results
    result_row_index = 0

    # Start time and record total time taken
    start_time = time.time()
    trial = 1
    best_trial = 1
    for i in range(len(lg)):
        for j in range(len(lr)):
            for k in range(len(aggr)):
                for l in range(len(dropout)):
                    for m in range(len(wd)):
                        for n in range(len(hgc)):  # Loop over hgc
                            args.lg = lg[i]
                            args.lr = lr[j]
                            args.aggr_method = aggr[k]
                            args.dropout = dropout[l]
                            args.wd = wd[m]
                            args.hgc = hgc[n]  # Set hgc parameter
                            args.trial_num = trial

                            # Create DataFrame to hold parameter values
                            param_df = pd.DataFrame([[lg[i], lr[j], aggr[k], dropout[l], wd[m], hgc[n]]], columns=['lg', 'lr', 'aggr', 'dropout', 'wd', 'hgc'])
                            # Write the parameter values to the Excel file
                            param_df.to_excel(excel_writer, sheet_name='Sheet1', startrow=result_row_index, startcol=0, index=False, header=True)
                            ...
                            for o_fold in range(1,6):
                                print('-------------- Processing data - outer_fold #:{} - trial {}'.format(o_fold, trial))
                                args.outer_loop = o_fold

                                val_results, test_results  = main(args)
                                acc.append(val_results[0])
                                foldsSc.iloc[o_fold-1,:] = val_results
                                foldsSc2.iloc[o_fold-1,:] = test_results

                            
                            # Update best accuracy and corresponding parameters if current accuracy is higher
                            acc = np.mean(acc)
                            if acc > best_accuracy:
                                best_accuracy = acc
                                best_trial = trial
                                best_params = [o_fold, lg[i], lr[j], aggr[k], dropout[l], wd[m], hgc[n], acc*100, result_row_index, best_trial]
    ...
    # End time and print total time taken
    end_time = time.time()
    print("Total time taken: {} seconds".format(end_time - start_time))

    # Save the Excel file
    excel_writer._save()
    # Save the best parameters to the Excel file
    best_params_df = pd.DataFrame([best_params], columns=['fold', 'lg', 'lr', 'aggr', 'dropout', 'wd', 'hgc', 'acc', 'row', 'trial'])
    with pd.ExcelWriter(args.RES_DIR + 'results.xlsx', mode='a', engine='openpyxl') as writer:
        best_params_df.to_excel(writer, index=False, sheet_name='Best_Params', startrow=0, header=True)
    # Print the parameters corresponding to the highest accuracy
    print("Best parameters (fold, lg, lr, aggr, dropout, wd, hgc)", best_params, "with highest accuracy: ", best_accuracy)
